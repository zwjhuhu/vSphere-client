/*
 * ******************************************************
 * Copyright VMware, Inc. 2010-2017.  All Rights Reserved.
 * ******************************************************
 *
 ...
 *
 * <b>Command Line:</b>
 * run.bat com.vmware.vapp.OVFManagerImportVAppFromUrl --url [webserviceurl]
    ...
    */
    ...
    ManagedObjectReference getDatacenterOfDatastore(
        ...
        List<ObjectContent> oCont =
                vimPort.retrievePropertiesEx(serviceContent.getPropertyCollector(),
                        propertyFilterSpecs, new RetrieveOptions()).getObjects();
        if (oCont != null) {
    ...
    void importVApp() {
        ...
        try {
            ...
            ManagedObjectReference hostMor = results.get(host);
            if (hostMor == null) {
    ...
    void getVMDKFile(boolean put, String readFileLocation,
        ...
        try {
            ...
            while (bytesRead >= 0) {
                ...
                System.out.println("Bytes Written: " + bytesWrote);
                bytesAvailable = readBufferedInputStream.available();
                bufferSize = Math.min(bytesAvailable, maxBufferSize);
